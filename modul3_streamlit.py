...
try:
    ...
    if not os.path.exists(os.path.join(nltk_data_dir, "corpora/stopwords")):
        st.error("Failed to download NLTK stopwords. Please check network connection.")
...
if df is not None:
    ...
    # Text Summarization
        ...
        with col1:
            column = st.selectbox(
                ...
                ["fuel", "machine", "part", "price", "service", "others"],
                key="summarization_column"
            )
        ...
        with col2:
            sentiment = st.selectbox(
                ...
                ["negative", "neutral", "positive"],
                key="summarization_sentiment"
            )
        
        # Filter data based on user selection
        if st.button("Apply Filter"):
            st.session_state.filtered_df = df[df[column] == sentiment][['sentence']]
            st.session_state.filter_applied = True  # Set filter as applied
            st.success(f"Filtered to {len(st.session_state.filtered_df)} sentences with {sentiment} sentiment about {column}")
        
        # Use default filter if no filter has been applied yet
        if st.session_state.filtered_df is not None:
            filtered_df = st.session_state.filtered_df
            num_sentences = min(st.slider("Number of sentences in summary:", 3, 10, 5), len(filtered_df))
            ...
            if st.session_state.filter_applied: 
                ...
                st.info("Please apply a filter first to enable summary generation.")
    ...
    # Text Similarity
        ...
        with col2:
            sentiment = st.selectbox(
                "Select sentiment type:", 
                ["negative", "neutral", "positive"],
                key="similarity_sentiment"
            )
        
        # Filter data based on user selection
        if st.button("Apply Filter", key="similarity_filter"):
            st.session_state.filtered_df = df[df[column] == sentiment][['sentence']]
    ...
    # Text Clustering
        ...
        with col2:
            sentiment = st.selectbox(
                "Select sentiment type:", 
                ["negative", "neutral", "positive"],
                key="clustering_sentiment"
            )
        
        # Filter data based on user selection
        if st.button("Apply Filter", key="clustering_filter"):
            st.session_state.filtered_df = df[df[column] == sentiment][['sentence']]
            st.session_state.filter_applied = True
            st.success(f"Filtered to {len(st.session_state.filtered_df)} sentences with {sentiment} sentiment about {column}")
        ...
        st.write(f"Using {len(limited_df)} sentences for clustering.")

        # Choose clustering method
        clustering_method = st.selectbox(
            "Select Clustering Method:",
            ["K-Means", "DBSCAN", "Latent Dirichlet Allocation (LDA)", "Hierarchical Clustering"]
        )
        ...
        if clustering_method == "K-Means":
            ...
            with col2:
                # Add silhouette analysis option
                do_silhouette = st.checkbox("Perform silhouette analysis", value=True)

            if st.button("Apply K-Means Clustering"):
                with st.spinner("Applying K-Means clustering..."):
                    # Vectorize the text data
                    vectorizer = TfidfVectorizer(tokenizer=enhanced_tokenizer, max_features=1000)
                    tfidf_matrix = vectorizer.fit_transform(limited_df['sentence'])
                    ...
                    if do_silhouette and len(set(clusters)) > 1:
                        silhouette_avg = silhouette_score(tfidf_matrix, clusters)
                        st.success(f"Silhouette Score: {silhouette_avg:.4f} (closer to 1 is better)")

                    # Now clusters will have same length as limited_df
                    limited_df['cluster'] = clusters
                ...
                for i in range(num_clusters):
                    ...
                    if len(indices) > 0:
                        # Sum up TF-IDF values for each term across all documents in this cluster
                        cluster_terms = tfidf_matrix[indices].sum(axis=0)
                        
                        # Convert to 1D array
                        cluster_terms = np.asarray(cluster_terms).flatten()
                        
                        # Get top terms
                        top_indices = cluster_terms.argsort()[-10:][::-1]
                        top_terms = [feature_names[j] for j in top_indices]
                        
                        st.write(", ".join(top_terms))
                    else:
                        st.write("No documents in this cluster")

                # Display clustered sentences
                st.subheader("Clustered Sentences")
                for cluster in range(num_clusters):
                    with st.expander(f"Cluster {cluster} ({len(limited_df[limited_df['cluster'] == cluster])} sentences)"):
                        cluster_sentences = limited_df[limited_df['cluster'] == cluster]['sentence'].values
                        for i, sentence in enumerate(cluster_sentences[:10]):
                            st.write(f"{i+1}. {sentence}")
                        if len(cluster_sentences) > 10:
                            st.write(f"... and {len(cluster_sentences) - 10} more")
                ...
                plot_df = pd.DataFrame({
                    ...
                    'y': coords[:, 1],
                    'cluster': clusters,
                    'sentence': limited_df['sentence'].values
                })
            ...
            with col1:
                eps = st.slider("Epsilon (neighborhood distance):", 0.1, 1.0, 0.5, 0.1)
            
            with col2:
                min_samples = st.slider("Min Samples:", 2, 10, 5)

            if st.button("Apply DBSCAN Clustering"):
                with st.spinner("Applying DBSCAN clustering..."):
                    # Vectorize the text data
                    vectorizer = TfidfVectorizer(tokenizer=enhanced_tokenizer, max_features=1000)
                    tfidf_matrix = vectorizer.fit_transform(limited_df['sentence'])
                    ...
                    st.success(f"DBSCAN found {n_clusters} clusters and {np.sum(clusters == -1)} noise points")

                # Display clustered sentences
                st.subheader("Clustered Sentences")
                
                # First show noise points if any
                if -1 in set(clusters):
                    with st.expander(f"Noise Points ({np.sum(clusters == -1)} sentences)"):
                        noise_sentences = limited_df[limited_df['cluster'] == -1]['sentence'].values
                        for i, sentence in enumerate(noise_sentences[:10]):
                            st.write(f"{i+1}. {sentence}")
                        if len(noise_sentences) > 10:
                            st.write(f"... and {len(noise_sentences) - 10} more")
                
                # Then show actual clusters
                for cluster in sorted(set(clusters)):
                    if cluster != -1:  # Skip noise points as we've already shown them
                        with st.expander(f"Cluster {cluster} ({len(limited_df[limited_df['cluster'] == cluster])} sentences)"):
                            cluster_sentences = limited_df[limited_df['cluster'] == cluster]['sentence'].values
                            for i, sentence in enumerate(cluster_sentences[:10]):
                                st.write(f"{i+1}. {sentence}")
    ...
    # Word2Vec Analysis
        ...
        # Filter data based on user selection
        if st.button("Apply Filter", key="w2v_filter"):
            st.session_state.filtered_df = df[df[column] == sentiment][['sentence']]
            st.session_state.filter_applied = True
            st.session_state.w2v_model = None  # Reset model when filter changes
            st.success(f"Filtered to {len(st.session_state.filtered_df)} sentences with {sentiment} sentiment about {column}")
    ...
    # Distance Metrics Comparison
        ...
        with col2:
            sentiment = st.selectbox(
                "Select sentiment type:", 
                ["negative", "neutral", "positive"],
                key="distance_sentiment"
            )
        
        # Filter data based on user selection
        if st.button("Apply Filter", key="distance_filter"):
            st.session_state.filtered_df = df[df[column] == sentiment][['sentence']]
            st.session_state.filter_applied = True
            st.session_state.distance_matrices = None  # Reset matrices when filter changes
            st.success(f"Filtered to {len(st.session_state.filtered_df)} sentences with {sentiment} sentiment about {column}")
